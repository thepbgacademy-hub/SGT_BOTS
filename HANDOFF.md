...
Use `docs/superpowers/plans/2026-07-10-telegram-bot-persona-runtime-roadmap.md` as the active execution map for persona/runtime work. Continue with Phase 1 Ticket 1.3: define reviewed persona records for persona bots only: `Rori` (`concierge_general_academy_KB`), `Top Secret` (`verifier`), and `Insight` (`tutor`). Do not create persona records for `Cursive` (`document_wizard`) or `ShAzZaM!` (`form_wizard`); keep them in deterministic utility lanes. Keep `Condor` (`tax_legal_research`) as a display-only playground surface for this roadmap. Do not apply production changes unless explicitly requested.
