...
Run a live Rori Telegram smoke focused on direct contact-routing questions, especially enrollment help phrased as `who do I talk to`. Recheck prompts like `Who do I talk to about enrolling for classes?`, `Who do I talk to if I have a billing issue?`, `What can I study here?`, and `How much do the levels cost?` and confirm that contact questions now answer with the right room plus a clear next step instead of falling back into pricing or generic enrollment copy. Keep the deterministic boundary rules in place: ordinary off-topic prompts should still return the fixed Academy-only line, jailbreak prompts should still return the fixed boundary line, and only jailbreak / instruction-bypass attempts should be written to `playground_audit_events`. Keep the playground shared tables on the `playground_` prefix in `public`, keep Rori Academy content separate in the `rori` schema, and continue treating `codex/rori-academy-concierge` as the active playground stabilization branch until shared Top Secret and session/provider work is fully merged. Do not deploy invented, fallback, fixture, or placeholder data.
...
Rori now has a direct enrollment-contact route for `who do I talk to` questions. Instead of falling back into the generic enrollment-and-pricing page, that wording now routes to `Lobby DM to staff` with a clear instruction to open a DM there and ask an admin for help getting started. This was added after a failing bot-runtime test showed the general `Rori DM` matcher still won that phrasing; the matcher was tightened and the test now passes. Pricing, support-escalation, and programs replies still keep the lighter paragraph-shaped formatting from the prior slice. Focused bot-runtime tests, API lint, and API build all passed; the next step is a live Telegram smoke on the exact enrollment-contact wording.
