...
- 2026-07-15: Completed the local Rori wiki load preflight. The reviewed public bundle generated dedicated-`rori`-schema upsert SQL, and import validation, Supabase retrieval, and Rori grounding checks passed with 3 files and 29 tests. Remote migration confirmation, SQL apply, and Telegram smoke test remain pending because VPS/Supabase access timed out; no remote write was performed. See `docs/superpowers/plans/2026-07-15-rori-wiki-load-preflight.md`.
- 2026-07-17: Completed the remote Rori wiki runtime gate. VPS2 already had the dedicated `rori` tables and expected records: seven published visible wiki pages, two visible support rooms, and no events. A process inside the deployed SGT Bots backend used its live Supabase environment plus the `rori` PostgREST profile and received HTTP 200 with all seven expected page keys. Focused Rori import, repository, and grounding coverage passed with 29 tests; API lint passed. No database write or container restart was required. The remaining check is an interactive Telegram conversation smoke using the frozen Rori prompts.
